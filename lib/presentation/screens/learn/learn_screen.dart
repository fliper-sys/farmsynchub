...
class _LearnScreenState extends ConsumerState<LearnScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<LearningLesson> lessons =
        _learningLessons.where((LearningLesson lesson) {
      final String haystack =
          '${lesson.title} ${lesson.subtitle} ${lesson.overview} ${lesson.track} ${lesson.difficulty} ${lesson.steps.join(' ')} ${lesson.tools.join(' ')} ${lesson.questions.map((q) => '${q.prompt} ${q.options.join(' ')}').join(' ')}'
              .toLowerCase();
      if (query.isEmpty) return true;
      if (haystack.contains(query)) return true;
      final List<String> tokens =
          query.split(RegExp('\\\\s+')).where((t) => t.isNotEmpty).toList();
    ...
    final double lessonProgress = lessons.isEmpty
        ...
        : lessons
                .map((LearningLesson lesson) =>
                    _lessonProgress(learning, lesson))
    ...
    final double progress = totalActions == 0
        ? 0
        : ((lessonProgress * lessons.length) + completedPractices) /
            totalActions;
        ...
        sections: <Widget>[
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor:
                  const AlwaysStoppedAnimation<Color>(AppColors.primaryMid),
            ),
          ),
          const SizedBox(height: 18),
          SoftSectionTitle(
...
class LearnLessonDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final CertificateRecord? certificate =
        learning.certificateForLesson(lesson.id);
    final int reviewedQuestions = lesson.questions
        .where(
          (LessonQuestion question) => learning.reviewedQuestions
              .contains('${lesson.id}::${question.id}'),
        )
        .length;
    final double questionProgress = lesson.questions.isEmpty
        ? 0
        : reviewedQuestions / lesson.questions.length;
                ...
                child: AppButton.primary(
                  onPressed: completed
                      ...
                      : () async {
                          ...
                          final CertificateRecord? record = ref
...
class _AwardsBoard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GridView.count(
      ...
      children:
          awards.map((_AwardData award) => _AwardCard(award: award)).toList(),
    );
  }
}
            ...
            Row(
              children: <Widget>[
                Container(
                  ...
                  decoration: BoxDecoration(
                    color:
                        award.unlocked ? award.tint : theme.colorScheme.surface,
                  ...
                  child: AppButton.primary(
                    onPressed: _revealed
                        ...
                        : () async {
                            ...
                            final bool correct = _selectedIndex ==
...
class _MiniMeta extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color background = isDark
        ? Color.alphaBlend(color.withOpacity(0.24), theme.colorScheme.surface)
        : color;
...
double _lessonProgress(LearningState learning, LearningLesson lesson) {
  if (lesson.questions.isEmpty) {
    return learning.completedLessons.contains(lesson.id)
        ? 1
        : lesson.baseProgress;
  }
...
/// Looks up a lesson by id (e.g. for deep-linking from a notification).
LearningLesson? findLearningLessonById(String id) {
  for (final LearningLesson lesson in _learningLessons) {
    if (lesson.id == id) {
      return lesson;
    }
  }
...
final List<LearningLesson> _learningLessons = <LearningLesson>[
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        explanation:
            'Checking the top soil early helps you see whether the root zone actually needs water.',
      ),
      LessonQuestion(
        id: 'soil-2',
        ...
        explanation:
            'Young plants are more vulnerable to moisture stress, so they should be served first.',
      ),
      LessonQuestion(
        id: 'soil-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mulch protects the soil surface and reduces water loss from heat and wind.',
      ),
      LessonQuestion(
        id: 'soil-4',
        ...
        correctOptionIndex: 0,
        explanation:
            'Poor drainage can cause waterlogging, so you should check the flow paths and reduce watering.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'A dated calendar keeps each animal group on time and reduces missed doses.',
      ),
      LessonQuestion(
        id: 'livestock-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Separation makes monitoring treatment results and follow-up much easier.',
      ),
      LessonQuestion(
        id: 'livestock-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Those records help the team keep the health plan accurate and actionable.',
      ),
      LessonQuestion(
        id: 'livestock-4',
        ...
        correctOptionIndex: 0,
        explanation:
            'Many vaccines lose strength if they are not kept at the right temperature.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'The best buyer is not always the highest price; reliability and logistics matter too.',
      ),
      LessonQuestion(
        id: 'market-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'A good sale price can still produce poor profit if transport eats the margin.',
      ),
      LessonQuestion(
        id: 'market-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Sale records make future pricing and inventory decisions much easier.',
      ),
      LessonQuestion(
        id: 'market-4',
        prompt:
            'What extra cost can reduce your real profit even when the sale price looks good?',
        options: <String>[
        ...
        correctOptionIndex: 0,
        explanation:
            'Transport and handling expenses can significantly reduce the profit from a sale.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'A zig-zag route covers more plants and gives a better picture of field conditions.',
      ),
      LessonQuestion(
        id: 'pest-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'The underside of leaves is a common place to find early pest signs.',
      ),
      LessonQuestion(
        id: 'pest-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Confirming the pest or disease first helps prevent waste and wrong treatment.',
      ),
      LessonQuestion(
        id: 'pest-4',
        ...
        correctOptionIndex: 0,
        explanation:
            'Beneficial insects can help control pests, so identifying them prevents unnecessary spraying.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Rain and wind can wash away spray or drift it away from target crops.',
      ),
      LessonQuestion(
        id: 'weather-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Early harvesting helps maintain product quality and reduces heat stress.',
      ),
      LessonQuestion(
        id: 'weather-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Humidity and soil moisture show whether the field actually needs water.',
      ),
      LessonQuestion(
        id: 'weather-4',
        prompt:
            'Why should labour-heavy jobs move to cooler hours during heat stress?',
        options: <String>[
        ...
        correctOptionIndex: 0,
        explanation:
            'Cooler hours reduce fatigue and help the team work more safely and efficiently.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Sorting damaged items prevents them from reducing the quality of the whole batch.',
      ),
      LessonQuestion(
        id: 'post-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Shade and clean containers protect produce from heat and contamination.',
      ),
      LessonQuestion(
        id: 'post-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Cooler hours reduce heat stress and help preserve freshness.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Records make the farm easier to assess and support financing decisions.',
      ),
      LessonQuestion(
        id: 'record-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Separating income and expenses keeps the farm\'s financial picture clear.',
      ),
      LessonQuestion(
        id: 'record-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Reviewing records before the next cycle helps you improve the next plan.',
      ),
      LessonQuestion(
        id: 'record-4',
        prompt:
            'What helps prove what happened on the farm when you review records later?',
        options: <String>[
        ...
        correctOptionIndex: 0,
        explanation:
            'Supporting documents make the record clearer and easier to trust.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Stable routines help animals stay healthy and reduce stress.',
      ),
      LessonQuestion(
        id: 'feed-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Different age groups and uses need different feeding plans.',
      ),
      LessonQuestion(
        id: 'feed-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Body condition and appetite are early signs of feed or health issues.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    subtitle:
        'Simple ways to improve soil strength with organic matter and nutrient planning.',
    baseProgress: 0.19,
    duration: '9 min',
    tint: Color(0xFFE8F4D8),
    track: AppStrings.soilManagement,
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Decomposed manure is less harsh on crops and easier to manage in the field.',
      ),
      LessonQuestion(
        id: 'soil-fertility-2',
        prompt:
            'What helps prevent the same nutrients from being removed repeatedly?',
        options: <String>[
          'Crop rotation',
        ...
        correctOptionIndex: 0,
        explanation:
            'Crop rotation spreads nutrient demand across different plants and seasons.',
      ),
      LessonQuestion(
        id: 'soil-fertility-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Dark, crumbly soil often means the soil is holding organic matter well.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    subtitle:
        'Raise stronger seedlings before transplanting them to the main field.',
    baseProgress: 0.27,
    duration: '10 min',
    tint: Color(0xFFDFF1FF),
    track: AppStrings.cropAdvice,
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Hardening prepares seedlings for field conditions and improves survival.',
      ),
      LessonQuestion(
        id: 'nursery-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Gentle watering protects seeds and young plants from being displaced.',
      ),
      LessonQuestion(
        id: 'nursery-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Thinning reduces competition and gives healthier seedlings a better start.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    subtitle:
        'Simple steps that stop disease from moving between animals, people, and equipment.',
    baseProgress: 0.22,
    duration: '11 min',
    tint: Color(0xFFFFEBCF),
    track: AppStrings.animalHealth,
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Biosecurity is about prevention and limiting spread, not replacing treatment.',
      ),
      LessonQuestion(
        id: 'biosecurity-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Isolation helps protect the rest of the herd or flock from possible spread.',
      ),
      LessonQuestion(
        id: 'biosecurity-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Cleaning equipment reduces the chance of carrying pathogens to new areas.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Checking the soil helps you avoid watering when the crop does not need it.',
      ),
      LessonQuestion(
        id: 'drip-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Blocked or leaking lines reduce irrigation efficiency and can stress the crop.',
      ),
      LessonQuestion(
        id: 'drip-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Logs help you learn which irrigation schedule works best for the field.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Airflow and dryness reduce heat stress and disease pressure.',
      ),
      LessonQuestion(
        id: 'poultry-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Overcrowding increases stress and makes management harder.',
      ),
      LessonQuestion(
        id: 'poultry-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'These areas can quickly build up dirt, waste, and disease risk.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    questions: <LessonQuestion>[
      ...
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Comparing offers gives you a clearer picture of the best market option.',
      ),
      LessonQuestion(
        id: 'margin-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Receipt history highlights which products and buyers deliver better returns.',
      ),
    ],
    tools: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Apply mulch to reduce evaporation, moderate soil temperature, and keep weeds from emerging close to stems.',
    steps: <String>[
      'Spread 2-3cm of organic mulch evenly',
      'Keep mulch pulled away from plant stems',
      'Refresh when the surface layer starts to break down'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'mulch-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Organic mulch helps keep soil moist and reduces weed pressure while adding organic matter as it decomposes.',
      ),
      LessonQuestion(
        id: 'mulch-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mulch against stems can trap moisture and encourage rot or pests near the plant base.',
      ),
      LessonQuestion(
        id: 'mulch-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mulch should be refreshed once it becomes thin so it continues to protect soil and conserve moisture.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Combine fresh green waste and dry brown material, keep the pile moist, and turn it weekly for even breakdown.',
    steps: <String>[
      'Layer greens and browns in a pile',
      'Keep the compost moist but not waterlogged',
      'Turn the pile weekly to add air and speed decomposition'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'compost-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Balanced green and brown materials keep compost active without becoming too wet or smelly.',
      ),
      LessonQuestion(
        id: 'compost-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Turning keeps the pile aerated and speeds decomposition by feeding microbes oxygen.',
      ),
      LessonQuestion(
        id: 'compost-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'A wrung-out sponge moisture level supports microbes without causing rot or odors.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Pick seeds with known performance in your region and market, and confirm they match your soil, season, and customer demand.',
    steps: <String>[
      'Check days to maturity for your growing season',
      'Confirm disease resistance for local pests',
      'Choose a variety the market values'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'seed-1',
        prompt:
            'What is the most important reason to match seed variety to your climate?',
        options: <String>[
        ...
        correctOptionIndex: 0,
        explanation:
            'Climate-matched seeds are more likely to mature successfully within the available season.',
      ),
      LessonQuestion(
        id: 'seed-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Resistant varieties can lower the need for chemical treatments and reduce crop failure.',
      ),
      LessonQuestion(
        id: 'seed-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Buying seeds that meet market demand helps ensure sales and better prices.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Adjust the amount and timing of water as plants grow, weather changes, and roots deepen.',
    steps: <String>[
      'Water young plants lightly daily',
      'Check soil moisture before each irrigation',
      'Reduce frequency as roots deepen and soil holds water longer'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'water-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Young plants have shallow roots and need careful moisture balance to avoid drowning or washing away.',
      ),
      LessonQuestion(
        id: 'water-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Soil moisture gives the most reliable sign of whether the crop needs irrigation.',
      ),
      LessonQuestion(
        id: 'water-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mature crops with deeper roots can access more water and often need less frequent irrigation.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Keep fields and equipment clean to lower pest pressure and stop disease from spreading between plants or animals.',
    steps: <String>[
      'Collect debris and burn or compost it away from crops',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Cleaning tools helps prevent moving pests and disease from one area to another.',
      ),
      LessonQuestion(
        id: 'sanitation-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Debris can hide pests and disease organisms that attack the next crop.',
      ),
      LessonQuestion(
        id: 'sanitation-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Standing water can become a breeding ground for mosquitoes and other pests.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Prune away dead or crowded branches to improve air movement, reduce disease risk, and help the plant focus energy on fruit or flower production.',
    steps: <String>[
      'Cut back dead wood first',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Thinning crowded branches allows light and air to reach the plant, reducing disease pressure.',
      ),
      LessonQuestion(
        id: 'prune-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Removing dead or diseased wood helps keep the rest of the plant healthy.',
      ),
      LessonQuestion(
        id: 'prune-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Clean cuts are less likely to tear and invite pests or disease into the plant.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Create a smooth, firm seedbed so seeds are planted at an even depth and young roots can establish quickly.',
    steps: <String>[
      'Clear debris and stones from the planting area',
      'Level the seedbed and firm the surface gently',
      'Check that the bed is not too compacted before sowing'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'seedbed-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'A firm seedbed helps seeds stay at the right depth while still allowing roots and water to move through.',
      ),
      LessonQuestion(
        id: 'seedbed-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Stones and debris can create uneven planting depth and obstacles for young roots.',
      ),
      LessonQuestion(
        id: 'seedbed-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Even seed depth supports consistent moisture and temperature for all seeds.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Match a healthy scion and rootstock, make clean cuts, and keep the graft protected until it heals.',
    steps: <String>[
      'Prepare matching scion and rootstock materials',
      'Make clean, angled cuts for contact points',
      'Bind and cover the graft to keep it clean and moist'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'graft-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Compatibility and clean contact points are essential for a successful graft union.',
      ),
      LessonQuestion(
        id: 'graft-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Clean cuts improve contact and reduce the chance of disease at the graft site.',
      ),
      LessonQuestion(
        id: 'graft-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Protection helps the graft heal and prevents drying or contamination.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Sow cover plants between cash crops to protect soil, reduce erosion, and add organic matter.',
    steps: <String>[
      'Select cover species that fit your climate',
      'Plant soon after harvest',
      'Manage the cover crop before it flowers and sets seed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'cover-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Cover crops protect the soil and improve fertility while the main crop is not growing.',
      ),
      LessonQuestion(
        id: 'cover-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Planting after harvest gives the cover crop a full growing window to protect soil.',
      ),
      LessonQuestion(
        id: 'cover-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Removing or mowing before seed set avoids creating a volunteer weed problem in the next season.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Look for damage patterns, spread, and visible signs like pests or spores to help identify the likely cause.',
    steps: <String>[
      'Inspect leaves and stems for patterns',
      'Compare symptoms across plants',
      'Check for visible pests or fungal growth'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'disease-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Pest damage and disease often look different, with pests leaving bite marks and disease causing spots or decay.',
      ),
      LessonQuestion(
        id: 'disease-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Comparing symptoms helps determine if the problem is general or isolated.',
      ),
      LessonQuestion(
        id: 'disease-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Fungal problems often produce visible spores, powdery surfaces, or mold.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Apply fertilizer at the right growth stages so plants can use the nutrients instead of losing them to runoff or leaching.',
    steps: <String>[
      'Identify the crop growth stage',
      'Choose application method for the stage',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Fertilizer is most effective when plants are in a growth stage that can absorb the nutrients.',
      ),
      LessonQuestion(
        id: 'fertilizer-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Heavy rain can wash fertilizer away before plants use it, reducing efficiency.',
      ),
      LessonQuestion(
        id: 'fertilizer-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Targeted applications place nutrients where young roots can access them without wasting material.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Inspect mulch regularly, refresh patches that have broken down, and remove it when it starts to trap moisture or pests against plant stems.',
    steps: <String>[
      'Inspect mulch every few weeks',
      'Top up thin areas',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Refreshing keeps mulch effective at conserving moisture and suppressing weeds.',
      ),
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Mulch too close to stems can trap moisture and invite rot or pests.',
      ),
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Decomposed mulch needs topping up to keep protecting the soil and suppressing weeds.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Walk irrigation lines regularly, look for leaking joints or blocked emitters, and repair problems before water is wasted.',
    steps: <String>[
      'Inspect lines weekly',
      'Clean or replace blocked emitters',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Regular checks prevent small issues from causing large water loss or uneven irrigation.',
      ),
      LessonQuestion(
        id: 'irrigation-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Blocked emitters cause some plants to receive too little water while others get too much.',
      ),
      LessonQuestion(
        id: 'irrigation-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Fixing leaks promptly conserves water and maintains proper irrigation pressure.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Use fine sprays or wick systems to keep nursery beds evenly moist without dislodging seeds or seedlings.',
    steps: <String>[
      'Use a fine mist or gentle watering tool',
      'Avoid heavy streams on seedlings',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Gentle watering keeps seeds in place and maintains moist contact with the soil.',
      ),
      LessonQuestion(
        id: 'nursery-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Heavy watering can damage delicate seedlings and displace seeds or soil.',
      ),
      LessonQuestion(
        id: 'nursery-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Watering based on soil moisture prevents both drying out and overwatering.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Observe the pest life cycle and focus control when pests are most vulnerable to minimize damage.',
    steps: <String>[
      'Identify the current pest stage',
      'Time control methods for vulnerable stages',
      'Record observations for future timing'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'pestlife-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Targeting pests when they are weakest improves control and lowers inputs.',
      ),
      LessonQuestion(
        id: 'pestlife-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Many pests are most vulnerable before they become adults or before they have hardened off.',
      ),
      LessonQuestion(
        id: 'pestlife-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Records help you predict when pests will appear and when control is best applied.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Dry, bale, or ensile excess fodder so you have quality feed during lean seasons.',
    steps: <String>[
      'Dry to a safe moisture level',
      'Bale or store under cover',
      'Protect from rodents and rain'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'fodder-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Dry fodder is less likely to rot and can be stored safely for longer.',
      ),
      LessonQuestion(
        id: 'fodder-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Covering and elevating prevents water damage and pest entry.',
      ),
      LessonQuestion(
        id: 'fodder-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Rodents damage and contaminate stored feed, reducing its value and safety.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Keep pond water clean, remove weeds, and check inflow and outflow so the system stays balanced.',
    steps: <String>[
      'Remove weeds and floating debris',
      'Check inlet and outlet flow',
      'Top up water level if needed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'pond-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Clearing weeds and debris keeps water moving and prevents clogging.',
      ),
      LessonQuestion(
        id: 'pond-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Good flow prevents stagnation and maintains water quality.',
      ),
      LessonQuestion(
        id: 'pond-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Maintaining normal water level helps the pond system stay stable and functional.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Reduce heat stress by assessing sun exposure and installing temporary shade or using natural shade trees.',
    steps: <String>[
      'Assess crop sun exposure',
      'Install shade cloth or structures',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Shade helps crops avoid heat damage and keeps them cooler during hot periods.',
      ),
      LessonQuestion(
        id: 'shade-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Monitoring ensures the shade is helping and not causing too much coolness or dampness.',
      ),
      LessonQuestion(
        id: 'shade-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Shade cloth and structures are practical ways to reduce direct sunlight on crops.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Review weekly notes, identify the most important insights, and list three clear actions to take next week.',
    steps: <String>[
      'List the top findings',
      'Decide three actions',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Summaries help turn raw observations into practical actions and decisions.',
      ),
      LessonQuestion(
        id: 'summary-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'A short list of actions is easier to follow and complete.',
      ),
      LessonQuestion(
        id: 'summary-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Assigning responsibility increases the chances that actions are completed.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Store seeds dry, cool, and sealed to maintain viability until planting season.',
    steps: <String>[
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Moisture can cause seeds to rot or sprout prematurely in storage.',
      ),
      LessonQuestion(
        id: 'seedstorage-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Airtight containers keep moisture out and protect seeds from pests.',
      ),
      LessonQuestion(
        id: 'seedstorage-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Cool, dry storage maintains seed quality and slows deterioration.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Focus on removing competitive and perennial weeds first so your crop can get the most light, water, and nutrients.',
    steps: <String>[
      'Identify the most competitive weed species',
      'Remove them before they set seed',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Competitive and perennial weeds cause the most damage over time and should be removed early.',
      ),
      LessonQuestion(
        id: 'weed-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Removing weeds before seed production stops the weed population from increasing.',
      ),
      LessonQuestion(
        id: 'weed-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Weeds often regrow, so monitoring helps you remove new shoots quickly.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Use covers, mulch, or water to moderate soil and air temperature when frost or cold nights are expected.',
    steps: <String>[
      'Cover crops at night',
      'Add mulch around roots',
      'Use light irrigation for frost protection when safe'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'cold-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Covers trap heat near the plant and reduce exposure to frost.',
      ),
      LessonQuestion(
        id: 'cold-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mulch helps keep soil temperature stable and protects roots from frost.',
      ),
      LessonQuestion(
        id: 'cold-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'A light protective irrigation can release heat as water freezes, helping protect plants.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Provide water, a diversity of flowers, and gentle handling to help bees thrive and pollinate your farm.',
    steps: <String>[
      'Offer clean water sources',
      'Plant different flowering species',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Bees need water and a variety of flowers to forage throughout the season.',
      ),
      LessonQuestion(
        id: 'bee-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Bees can be harmed by insecticides, so spraying when they are inactive is safer.',
      ),
      LessonQuestion(
        id: 'bee-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Different flowers provide nectar and pollen at different times of the season.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Plan crop rotations to balance nutrients, reduce pests and diseases, and improve soil health over time.',
    steps: <String>[
      'Choose different crop families for successive seasons',
      'Avoid planting the same crop in the same bed two years in a row',
      'Record past crops and rotation plans'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'rotation-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Different crop families host different pests and use nutrients differently, so rotation helps reduce problems.',
      ),
      LessonQuestion(
        id: 'rotation-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Knowing what was grown where helps plan effective rotations and avoid repeated problems.',
      ),
      LessonQuestion(
        id: 'rotation-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Without rotation, pests, diseases, and nutrient depletion can increase in a bed.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Provide clean water, balanced feed, and regular health checks to keep goats and sheep productive and healthy.',
    steps: <String>[
      'Check hooves and coat condition regularly',
      'Offer clean water daily',
      'Provide mineral supplements as needed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'ruminant-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Clean water is essential to health and digestion in goats and sheep.',
      ),
      LessonQuestion(
        id: 'ruminant-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Hoof problems can cause pain and reduce mobility, so regular checks prevent issues.',
      ),
      LessonQuestion(
        id: 'ruminant-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Mineral supplements fill nutritional gaps that can affect growth and reproduction.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Check greenhouse ventilation, shading, and irrigation distribution daily to keep conditions stable and crops healthy.',
    steps: <String>[
      'Open vents when it gets hot',
      'Adjust shade when light is intense',
      'Ensure drip lines or hoses are delivering evenly'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'greenhouse-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Good ventilation prevents overheating and reduces disease risk in greenhouse crops.',
      ),
      LessonQuestion(
        id: 'greenhouse-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Even water distribution avoids dry spots and overwatered areas.',
      ),
      LessonQuestion(
        id: 'greenhouse-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Shade helps protect crops from intense sun and keeps temperatures more stable.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Sanitize nursery trays and tools, keep spacing between seedlings, and remove infected plants quickly.',
    steps: <String>[
      'Sanitize trays and tools before use',
      'Space seedlings to improve air circulation',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Sanitizing reduces the chance that pathogens will infect young seedlings.',
      ),
      LessonQuestion(
        id: 'nursery-2-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Proper spacing keeps humidity lower and reduces the spread of disease.',
      ),
      LessonQuestion(
        id: 'nursery-2-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Removing diseased plants stops infection from spreading to healthy seedlings.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Take representative soil samples and use pH or kit tests to guide whether lime, fertilizer, or organic matter are needed.',
    steps: <String>[
      'Collect samples from several spots',
      'Label and test or send to a lab',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Multiple samples reduce the chance that one unusual spot will skew the results.',
      ),
      LessonQuestion(
        id: 'soiltest-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Soil pH affects nutrient availability and helps decide whether liming or acidifying is needed.',
      ),
      LessonQuestion(
        id: 'soiltest-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Keeping records helps you see whether management steps are improving soil health.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Remove crowded shoots and crossing branches to improve airflow, light, and fruit quality.',
    steps: <String>[
      'Thin crowded shoots',
      'Remove crossing branches',
      'Leave the strongest shoots'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'prune-prod-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Pruning removes crowded growth and helps the best shoots develop better fruit.',
      ),
      LessonQuestion(
        id: 'prune-prod-2',
        prompt:
            'Which branches should you remove when pruning for productivity?',
        options: <String>[
        ...
        correctOptionIndex: 0,
        explanation:
            'Crossing and weak growth reduce airflow and take energy from the best shoots.',
      ),
      LessonQuestion(
        id: 'prune-prod-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Leaving the best shoots helps the plant focus resources on quality production.',
      ),
    ],
    tools: <String>['Pruner'],
  ),
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Use padding, avoid overstacking, and separate varieties so produce arrives in market-ready condition.',
    steps: <String>[
      'Pad crates and boxes',
      'Avoid overstacking',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Padding cushions produce and prevents damage from movement.',
      ),
      LessonQuestion(
        id: 'packing-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Too much weight from overstacking can bruise or crush produce underneath.',
      ),
      LessonQuestion(
        id: 'packing-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Separating fragile items reduces the chance of damage during transport.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Calculate herd feed needs through the dry season and source or conserve fodder well before shortages occur.',
    steps: <String>[
      'Count herd size and type',
      'Multiply by daily intake needed',
      'Secure supply or grow cover crops',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Planning ahead ensures feed is available when pasture is scarce.',
      ),
      LessonQuestion(
        id: 'winterfeed-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Knowing your herd size and intake helps calculate total needs.',
      ),
      LessonQuestion(
        id: 'winterfeed-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Dry storage keeps feed from spoiling and protects it from pests.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Plant a variety of flowering species that bloom at different times to provide continuous nectar for bees.',
    steps: <String>[
      'Choose early, mid, and late blooming plants',
      'Plant in clusters',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
        ...
        correctOptionIndex: 0,
        explanation:
            'Different plants bloom at different times, giving bees consistent food.',
      ),
      LessonQuestion(
        id: 'beeplant-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Pesticides harm bees, so spraying during bloom risks bee populations.',
      ),
      LessonQuestion(
        id: 'beeplant-3',
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Grafts are vulnerable after joining. Shade, keep moist, and monitor for healing and failure.',
    steps: <String>[
      'Shade grafts from direct sun',
      'Keep soil consistently moist',
      'Inspect daily for wilting or separation',
      'Remove binding once healed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'graft-1',
        ...
        correctOptionIndex: 0,
        explanation:
            'Shade reduces water loss and heat stress on the delicate graft union while it heals.',
      ),
      LessonQuestion(
        id: 'graft-2',
        ...
        correctOptionIndex: 0,
        explanation:
            'Wilting and lack of callus (the healing tissue) indicate the graft is not taking.',
      ),
      LessonQuestion(
        id: 'graft-3',
        ...
        correctOptionIndex: 0,
        explanation:
            'Once callus tissue forms and the union is strong enough, remove binding to prevent girdling.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Steep finished compost in water with aeration to activate beneficial microbes. Use the resulting tea as a foliar spray to boost plant immunity and nutrient uptake.',
    steps: <String>[
      'Fill bucket with water',
      'Add finished compost and aerate 24-48 hours',
      'Strain through cloth',
      'Dilute and spray on leaf undersides at dawn'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
        id: 'comptea-1',
        ...
        options: <String>[
          'Provides beneficial microbes and slow-release nutrients',
          'Replaces all pest management',
          'Makes soil waterproof'
        ],
        correctOptionIndex: 0,
        explanation:
            'Compost tea delivers living microbes that improve soil health and nutrient availability.',
      ),
      LessonQuestion(
        id: 'comptea-2',
        prompt: 'How long should compost tea brew?',
        options: <String>[
          '24-48 hours with aeration',
          '5 minutes',
          'One week in sunlight'
        ],
        correctOptionIndex: 0,
        explanation:
            'Aeration during 24-48 hours allows beneficial microbes to multiply significantly.',
      ),
      LessonQuestion(
        id: 'comptea-3',
        prompt: 'When is the best time to spray?',
        options: <String>[
          'Early morning when leaves are damp',
          'Midday in full sun',
          'During rain'
        ],
        correctOptionIndex: 0,
        explanation:
            'Early morning spray allows microbes to establish before sun stress kills them.',
      ),
    ],
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Bare soil erodes in rain and loses nutrients. Cover with residues or live crops to protect structure.',
    steps: <String>[
      'Collect crop residues after harvest',
      'Spread 5-10 cm layer evenly',
      'Plant cover crops if area stays bare'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sc-1',
          prompt: 'Why is bare soil a problem?',
          options: <String>[
            'Erodes and loses nutrients',
            'Stores water too well',
            'Reduces pest habitat'
          ],
          correctOptionIndex: 0,
          explanation: 'Rain breaks soil; water washes away topsoil.'),
      LessonQuestion(
          id: 'sc-2',
          prompt: 'Good soil cover source?',
          options: <String>['Crop residues or plants', 'Plastic only', 'Rocks'],
          correctOptionIndex: 0,
          explanation: 'Organic improves soil.'),
      LessonQuestion(
          id: 'sc-3',
          prompt: 'How deep should cover be?',
          options: <String>['5-10 cm', 'Barely visible', 'Over 20 cm'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Before selling, confirm price, buyer willingness, and transport costs to avoid losses.',
    steps: <String>[
      'Contact 3+ buyers for quotes',
      'Confirm volume and payment',
      'Calculate transport vs profit'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'mk-1',
          prompt: 'Why check multiple buyers?',
          options: <String>[
            'Prices vary; avoid selling low',
            'Each reliable',
            'Waste time'
          ],
          correctOptionIndex: 0,
          explanation: 'Prices fluctuate; checking ensures competitive rates.'),
      LessonQuestion(
          id: 'mk-2',
          prompt: 'Before selling, important?',
          options: <String>[
            'Confirm buyer will buy',
            'Guess they have money',
            'Assume'
          ],
          correctOptionIndex: 0,
          explanation: 'Reliability matters; confirm terms.'),
      LessonQuestion(
          id: 'mk-3',
          prompt: 'Subtract from market price?',
          options: <String>['Transport, fees', 'Nothing', 'Water weight'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Seedlings need gradual exposure to sun/wind to avoid transplant shock.',
    steps: <String>[
      'Start 7-10 days before transplant',
      'Place in shade 2 hours daily',
      'Increase sun and reduce water'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'hd-1',
          prompt: 'What is hardening?',
          options: <String>[
            'Exposing to stress gradually',
            'Making hard',
            'Stopping water'
          ],
          correctOptionIndex: 0,
          explanation: 'Acclimates to sun, wind.'),
      LessonQuestion(
          id: 'hd-2',
          prompt: 'When start hardening?',
          options: <String>['7-10 days before', 'Day before', 'After'],
          correctOptionIndex: 0,
          explanation: 'Early prevents shock.'),
      LessonQuestion(
          id: 'hd-3',
          prompt: 'Without hardening?',
          options: <String>['Wilt and shock', 'Grow faster', 'Need no water'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    steps: <String>[
      'Walk systematically, mark areas',
      'Note type and intensity',
      'Plan control per area'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'wm-1',
          prompt: 'Why map weeds?',
          options: <String>[
            'Target control efficiently',
            'Kills auto',
            'Just recording'
          ],
          correctOptionIndex: 0,
          explanation: 'Shows priority areas.'),
      LessonQuestion(
          id: 'wm-2',
          prompt: 'Record what?',
          options: <String>['Location, type, intensity', 'Field name', 'Crop'],
          correctOptionIndex: 0,
          explanation: 'Guides control choices.'),
      LessonQuestion(
          id: 'wm-3',
          prompt: 'Next season benefit?',
          options: <String>['Compare patterns', 'No benefit', 'This year only'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Simple records show health patterns, treatment effectiveness, and trends.',
    steps: <String>[
      'Record date, group, observation',
      'Note treatments and costs',
      'Record outcome 3-7 days later'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'lr-1',
          prompt: 'Main benefit of records?',
          options: <String>[
            'Identify patterns and effectiveness',
            'No value',
            'Government only'
          ],
          correctOptionIndex: 0,
          explanation: 'Show what works, what recurs.'),
      LessonQuestion(
          id: 'lr-2',
          prompt: 'Record what?',
          options: <String>[
            'Date, group, obs, treatment, outcome',
            'Deaths',
            'Costs'
          ],
          correctOptionIndex: 0,
          explanation: 'Complete tracks trends.'),
      LessonQuestion(
          id: 'lr-3',
          prompt: 'Review how often?',
          options: <String>['Monthly for patterns', 'Never', 'Yearly'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Trees reduce heat, provide windbreak, shelter, and soil improvement over time.',
    steps: <String>[
      'Choose fast-growing species',
      'Plant north/west for afternoon shade',
      'Space 4-6 meters apart',
      'Water 2 years until established'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'st-1',
          prompt: 'Why shade trees?',
          options: <String>[
            'Reduce heat, wind, improve soil',
            'No maintenance',
            'Immediate'
          ],
          correctOptionIndex: 0,
          explanation: 'Long-term benefits multiple ways.'),
      LessonQuestion(
          id: 'st-2',
          prompt: 'When choosing?',
          options: <String>['Select fast-growing', 'Any tree', 'Slow'],
          correctOptionIndex: 0,
          explanation: 'Affects rate and benefit timing.'),
      LessonQuestion(
          id: 'st-3',
          prompt: 'When benefit?',
          options: <String>['After 3-5 years', 'Immediately', 'After 20+'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Compacted soil restricts roots and water. Break compaction mechanically or biologically.',
    steps: <String>[
      'Avoid traffic when wet',
      'Use deep tools or aerators',
      'Add organic matter',
      'Grow deep roots'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ae-1',
          prompt: 'Problem with compact soil?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Restricts root and water.'),
      LessonQuestion(
          id: 'ae-2',
          prompt: 'When avoid traffic?',
          options: <String>['When wet compacts', 'Winter only', 'Never'],
          correctOptionIndex: 0,
          explanation: 'Wet soil compresses easily.'),
      LessonQuestion(
          id: 'ae-3',
          prompt: 'Improve biologically?',
          options: <String>[
            'Add organic, grow roots',
            'More fertilizer',
            'Remove organics'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Regular checks prevent escapes, theft, and predator access to animals.',
    steps: <String>[
      'Walk weekly looking for damage',
      'Check holes, posts, rust',
      'Repair immediately',
      'Replace weak sections seasonally'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'fc-1',
          prompt: 'Why regular checks?',
          options: <String>[
            'Small holes grow; prevent escape',
            'Never need repair',
            'After escape'
          ],
          correctOptionIndex: 0,
          explanation: 'Early catches prevents loss.'),
      LessonQuestion(
          id: 'fc-2',
          prompt: 'Look for what?',
          options: <String>[
            'Holes, posts, rust, damage',
            'Only color',
            'Nothing'
          ],
          correctOptionIndex: 0,
          explanation: 'These signs weakness.'),
      LessonQuestion(
          id: 'fc-3',
          prompt: 'When repair?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Use shade, racks, and airflow to dry safely and reduce spoilage.',
    steps: <String>[
      'Select mature healthy produce',
      'Spread thin on clean racks',
      'Place in shaded ventilated area',
      'Turn regularly for even drying'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'dr-1',
          prompt: 'Why shade drying?',
          options: <String>[
            'Prevents bleaching and retains nutrients',
            'Sun faster',
            'Shade hard'
          ],
          correctOptionIndex: 0,
          explanation: 'Protects color and nutrients.'),
      LessonQuestion(
          id: 'dr-2',
          prompt: 'Why turn regularly?',
          options: <String>[
            'Ensures even drying, no mold',
            'Makes harder',
            'No reason'
          ],
          correctOptionIndex: 0,
          explanation: 'Improves air circulation.'),
      LessonQuestion(
          id: 'dr-3',
          prompt: 'Produce dry enough?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Proper maintenance extends tool life and improves work efficiency.',
    steps: <String>[
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'tc-1',
          prompt: 'Why sharpen regularly?',
          options: <String>[
            'Need less effort and faster',
            'Make noise',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Sharp is safer and faster.'),
      LessonQuestion(
          id: 'tc-2',
          prompt: 'When oil tools?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Oil prevents rust.'),
      LessonQuestion(
          id: 'tc-3',
          prompt: 'Best storage?',
          options: <String>['Dry location protected', 'Outside', 'In water'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Clear labeling prevents wrong variety transplants and tracks performance.',
    steps: <String>[
      'Record variety and date on label',
      'Use waterproof marker and stake',
      'Place at row start and end',
      'Check regularly for legibility'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'nl-1',
          prompt: 'Why label nursery?',
          options: <String>[
            'Prevents wrong varieties',
            'Just suggestion',
            'Big farms'
          ],
          correctOptionIndex: 0,
          explanation: 'Ensures correct transplant.'),
      LessonQuestion(
          id: 'nl-2',
          prompt: 'What on labels?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Both track age and performance.'),
      LessonQuestion(
          id: 'nl-3',
          prompt: 'Why waterproof?',
          options: <String>[
            'Survives nursery water',
            'Regular ok',
            'Color irrelevant'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Traps monitor populations and reduce pest numbers with low cost.',
    steps: <String>[
      'Set traps at dusk near crops',
      'Inspect daily and record count',
      'Empty and refresh regularly',
      'Use data to time control'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pt-1',
          prompt: 'Main benefit of traps?',
          options: <String>[
            'Monitor populations and time control',
            'Better than all',
            'Just decoration'
          ],
          correctOptionIndex: 0,
          explanation: 'Shows what and when.'),
      LessonQuestion(
          id: 'pt-2',
          prompt: 'When set?',
          options: <String>['At dusk when active', 'Midday', 'Never work'],
          correctOptionIndex: 0,
          explanation: 'Many active at dusk.'),
      LessonQuestion(
          id: 'pt-3',
          prompt: 'Why record?',
          options: <String>[
            'Data shows control timing',
            'Just paperwork',
            'Irrelevant'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    steps: <String>[
      'Use netting 1.5-2 meters high',
      'Bury bottom 15 cm prevent dig',
      'Provide sheltered roosting area',
      'Check daily for damage'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pf-1',
          prompt: 'How high fencing?',
          options: <String>['1.5-2 meters prevent', '30 cm', 'Irrelevant'],
          correctOptionIndex: 0,
          explanation: 'Adequate prevents escape.'),
      LessonQuestion(
          id: 'pf-2',
          prompt: 'Why bury bottom?',
          options: <String>[
            'Prevents digging and escape',
            'Just cosmetic',
            'Wastes'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Clear reporting builds trust with buyers and partners through transparency.',
    steps: <String>[
      'Prepare one-page summary',
      'Include dates and quantities',
      'Attach receipts or photos',
      'Send by phone or email'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'rs-1',
          prompt: 'Why share records?',
          options: <String>[
      ...
      LessonQuestion(
          id: 'rs-2',
          prompt: 'Include what?',
          options: <String>['Key numbers dates evidence', 'Vague', 'No data'],
          correctOptionIndex: 0,
          explanation: 'Specific credible.'),
      LessonQuestion(
          id: 'rs-3',
          prompt: 'How detailed?',
          options: <String>['One page summary', 'Book-length', 'Verbal'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Weekly checks reveal compost progress and readiness through temp and moisture.',
    steps: <String>[
      'Feel pile temperature from center',
      'Check moisture like squeezed sponge',
      'Smell for ammonia or dry scent',
      'Turn if too cool or wet'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'cc-1',
          prompt: 'Hot pile indicates?',
          options: <String>[
            'Microbes breaking down active',
            'Pile burned',
            'No activity'
          ],
          correctOptionIndex: 0,
          explanation: 'Heat shows activity.'),
      LessonQuestion(
          id: 'cc-2',
          prompt: 'Right moisture?',
          options: <String>['Moist like sponge', 'Bone dry', 'Soaking'],
          correctOptionIndex: 0,
          explanation: 'Ideal for activity.'),
      LessonQuestion(
          id: 'cc-3',
          prompt: 'Ammonia smell?',
          options: <String>['Pile too wet turn', 'Pile ready', 'Pile burning'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Grouping by age simplifies feeding and improves health monitoring consistency.',
    steps: <String>[
      'Separate young from adults',
      'Keep different purpose separate',
      'Tag or mark groups clearly',
      'Maintain consistent feed schedule'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'hg-1',
          prompt: 'Why group by age?',
          options: <String>[
            'Similar ages same feed needs',
            'All same',
            'Wastes time'
          ],
          correctOptionIndex: 0,
      ...
      LessonQuestion(
          id: 'hg-2',
          prompt: 'What separate?',
          options: <String>[
            'Young from adults different',
            'No separation',
            'Only color'
          ],
          correctOptionIndex: 0,
          explanation: 'Prevents bullying.'),
      LessonQuestion(
          id: 'hg-3',
          prompt: 'Why label?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Test moisture content and assess quality to decide whether to sell fresh or dry produce.',
    steps: <String>[
      'Break sample to check interior dryness',
      'Weigh to track moisture loss',
      'Assess for mold or damage',
      'Choose market route based on condition'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'da-1',
          prompt: 'How to test if produce is dry enough?',
          options: <String>[
            'Break sample; interior should be dry and brittle',
            'Only check surface',
            'Use smell alone'
          ],
          correctOptionIndex: 0,
          explanation: 'Interior dryness ensures shelf stability.'),
      LessonQuestion(
          id: 'da-2',
          prompt: 'Why track moisture loss over time?',
          options: <String>[
            'Shows when drying is complete',
            'Not important',
            'Just for records'
          ],
          correctOptionIndex: 0,
          explanation: 'Weight loss directly indicates moisture removal.'),
      LessonQuestion(
          id: 'da-3',
          prompt: 'Best market for high-moisture produce?',
          options: <String>[
            'Sell fresh immediately to avoid loss',
            'Force dry anyway',
            'Discard'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Different crops need different shade levels. Choose percentage based on crop tolerance and climate.',
    steps: <String>[
      'Identify crop sun tolerance (full/partial/shade)',
      'Check climate heat intensity',
      'Select 30-70% shade cloth',
      'Monitor and adjust if needed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sc-1',
          prompt: 'What shade for sun-loving crops?',
          options: <String>['30-40% shade', '70% shade', 'No shade'],
          correctOptionIndex: 0,
          explanation: 'Light shade protects from extreme sun.'),
      LessonQuestion(
          id: 'sc-2',
          prompt: 'Why vary shade percentage?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Each crop has optimal light requirements.'),
      LessonQuestion(
          id: 'sc-3',
          prompt: 'When increase shade?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Each crop shows readiness through size, color, and firmness. Learn variety-specific indicators.',
    steps: <String>[
      'Check size against variety standard',
      'Look for color change from unripe',
      'Feel firmness; ripe yields slightly to pressure',
      'Taste sample if appropriate'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ht-1',
          prompt: 'Main harvest readiness signs?',
          options: <String>[
            'Size, color, and firmness together',
            'Only size matters',
            'Color alone'
          ],
          correctOptionIndex: 0,
          explanation: 'Multiple indicators confirm ripeness.'),
      LessonQuestion(
          id: 'ht-2',
          prompt: 'Why sample before full harvest?',
          options: <String>[
            'Confirm ripeness throughout field',
            'Wastes time',
            'Not necessary'
          ],
          correctOptionIndex: 0,
          explanation: 'Field maturity varies; sampling guides decisions.'),
      LessonQuestion(
          id: 'ht-3',
          prompt: 'Underripe vs overripe?',
          options: <String>[
            'Pick underripe; prevents waste and ripens post-harvest',
            'Overripe better',
            'Timing irrelevant'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Clean storage areas prevent pest infestations and microbial spoilage of stored products.',
    steps: <String>[
      'Remove all debris and old produce',
      'Sweep and wash floors and walls',
      'Elevate fresh produce on pallets',
      'Seal cracks and gaps'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sh-1',
          prompt: 'Why remove old debris?',
          options: <String>[
            'Harbors pests and mold spores',
            'Just cosmetic',
            'No real benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Debris is food source for pests.'),
      LessonQuestion(
          id: 'sh-2',
          prompt: 'Why elevate on pallets?',
          options: <String>[
            'Prevents floor moisture and pest access',
            'Just easier',
            'No reason'
          ],
          correctOptionIndex: 0,
          explanation: 'Air circulation and separation prevents contact rot.'),
      LessonQuestion(
          id: 'sh-3',
          prompt: 'When clean storage?',
          options: <String>[
            'Before each season and after use',
            'Never',
            'Once yearly'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Collect rainwater runoff into tanks or ponds for supplemental irrigation in dry periods.',
    steps: <String>[
      'Calculate catchment area and rainfall',
      'Size tank for storage',
      'Install gutters and pipes',
      'Include simple filtration'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'wh-1',
          prompt: 'Best use of harvested water?',
          options: <String>[
            'Supplemental irrigation during dry season',
            'Drinking only',
            'Impossible to use'
          ],
          correctOptionIndex: 0,
          explanation: 'Water storage extends irrigation capability.'),
      LessonQuestion(
          id: 'wh-2',
          prompt: 'Why calculate catchment area?',
          options: <String>[
            'Determines tank size needed',
            'Not important',
            'Guess size'
          ],
          correctOptionIndex: 0,
          explanation: 'Area × rainfall = volume captured.'),
      LessonQuestion(
          id: 'wh-3',
          prompt: 'Important for water storage?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Proper wound care prevents infection and reduces animal suffering and recovery time.',
    steps: <String>[
      'Restrain animal calmly',
      'Flush wound with clean water',
      'Apply antiseptic solution',
      'Bandage if needed; monitor for infection'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'af-1',
          prompt: 'First step for wounds?',
          options: <String>[
            'Clean with water to remove dirt',
            'Apply dressing immediately',
            'Wait for vet'
          ],
          correctOptionIndex: 0,
          explanation: 'Cleaning prevents infection.'),
      LessonQuestion(
          id: 'af-2',
          prompt: 'Why use antiseptic?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Antiseptic reduces disease risk.'),
      LessonQuestion(
          id: 'af-3',
          prompt: 'Monitor bandage for?',
          options: <String>[
            'Swelling, discharge, or odor indicating infection',
            'Changes in weather',
            'Color change only'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Remove excess plants early to give remaining plants space, water, and light.',
    steps: <String>[
      'Thin when plants 5-10 cm high',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ct-1',
          prompt: 'Why thin early?',
          options: <String>[
            'Reduces competition and allows growth',
            'Harder later',
            'Never needed'
          ],
          correctOptionIndex: 0,
          explanation: 'Early thinning prevents stunting.'),
      LessonQuestion(
          id: 'ct-2',
          prompt: 'What plants keep?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Best plants produce better yields.'),
      LessonQuestion(
          id: 'ct-3',
          prompt: 'After thinning, do what?',
          options: <String>['Water to settle soil', 'Nothing', 'Thin again'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Local materials like compost and manure improve soil structure and add nutrients cheaply.',
    steps: <String>[
      'Collect compost, manure, or residues',
      'Test soil to guide amendment type',
      'Apply 5-10 cm layer',
      'Mix into topsoil and water in'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sa-1',
          prompt: 'Best local amendment?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Local organics are cheap and beneficial.'),
      LessonQuestion(
          id: 'sa-2',
          prompt: 'Why test soil?',
          options: <String>[
            'Identifies deficiencies to address',
            'Wastes time',
            'Guess amendments'
          ],
          correctOptionIndex: 0,
          explanation: 'Testing guides amendment choices.'),
      LessonQuestion(
          id: 'sa-3',
          prompt: 'Application depth?',
          options: <String>['5-10 cm layer mixed in', 'Surface only', 'Deep'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Proper handling and timing dramatically improves survival and crop establishment.',
    steps: <String>[
      'Harden seedlings 7-10 days before',
      'Transplant in cool part of day',
      'Water soil well before digging',
      'Plant at same depth; water in gently'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'tc-1',
          prompt: 'Before transplanting, do what?',
          options: <String>[
            'Harden seedlings gradually to stress',
            'Plant immediately indoors',
            'Skip this'
          ],
          correctOptionIndex: 0,
          explanation: 'Hardening prevents shock.'),
      LessonQuestion(
          id: 'tc-2',
          prompt: 'When transplant?',
          options: <String>[
            'Cool part of day or cloudy',
            'Midday sun',
            'Anytime'
          ],
          correctOptionIndex: 0,
          explanation: 'Cool conditions reduce wilt.'),
      LessonQuestion(
          id: 'tc-3',
          prompt: 'Correct planting depth?',
          options: <String>['Same depth as in nursery', 'Deeper', 'Shallower'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Balance roughage, protein, and energy to meet animal nutritional requirements efficiently.',
    steps: <String>[
      'Estimate daily intake for each animal',
      'Know basic protein percentages needed',
      'Mix grains, legumes, roughage proportions',
      'Record formula for consistency'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'fm-1',
          prompt: 'Main feed components?',
          options: <String>[
            'Roughage, grain, legume for balance',
            'Only one type',
            'Random mix'
          ],
          correctOptionIndex: 0,
          explanation: 'Balance provides complete nutrition.'),
      LessonQuestion(
          id: 'fm-2',
          prompt: 'Why record formula?',
          options: <String>[
            'Ensures consistent nutrition and results',
            'Not needed',
            'Just mixing'
          ],
          correctOptionIndex: 0,
          explanation: 'Records prevent errors.'),
      LessonQuestion(
          id: 'fm-3',
          prompt: 'Protein need varies by?',
          options: <String>[
            'Age, production level, health',
            'Never varies',
            'Only weight'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Understanding your costs and having alternative buyers increases negotiating power.',
    steps: <String>[
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'mn-1',
          prompt: 'Why calculate minimum price?',
          options: <String>[
            'Know when negotiations are unprofitable',
            'Random guess',
            'Never needed'
          ],
          correctOptionIndex: 0,
          explanation: 'Knowing costs prevents losing money.'),
      LessonQuestion(
          id: 'mn-2',
          prompt: 'Best negotiation approach?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Competition increases your negotiating power.'),
      LessonQuestion(
          id: 'mn-3',
          prompt: 'When to walk away?',
          options: <String>[
            'If offer is below minimum',
            'Never',
            'Always push harder'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Cover crops prevent erosion, suppress weeds, and legumes fix nitrogen affordably.',
    steps: <String>[
      'Select appropriate crop for season',
      'Plant after harvest',
      'Grow until next season',
      'Plow or cut and incorporate'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'cc-1',
          prompt: 'Best cover crop benefit?',
          options: <String>[
            'Legumes fix nitrogen cheaply',
            'No benefit',
            'Waste of time'
          ],
          correctOptionIndex: 0,
          explanation: 'Nitrogen fixation reduces fertilizer cost.'),
      LessonQuestion(
          id: 'cc-2',
          prompt: 'When plant cover crop?',
          options: <String>[
            'After harvest before dry season',
            'Anytime',
            'Never needed'
          ],
          correctOptionIndex: 0,
          explanation: 'Timing maximizes growth and benefit.'),
      LessonQuestion(
          id: 'cc-3',
          prompt: 'Why erosion control matters?',
          options: <String>[
            'Protects soil from rain, wind damage',
            'Cosmetic only',
            'No benefit'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'A pre-loading checklist prevents forgotten items and buyer miscommunication.',
    steps: <String>[
      'Count and verify crate quantities',
      'Check product quality and ripeness',
      'Confirm buyer details and price',
      'Verify transport availability'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sp-1',
          prompt: 'Checklist prevents?',
          options: <String>[
            'Forgotten items and mistakes',
            'Wasting time',
            'Has no benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Organization prevents costly errors.'),
      LessonQuestion(
          id: 'sp-2',
          prompt: 'What to verify before loading?',
          options: <String>[
            'Counts, quality, buyer contact, transport',
            'Skip checks',
            'Load randomly'
          ],
          correctOptionIndex: 0,
          explanation: 'Verification ensures customer satisfaction.'),
      LessonQuestion(
          id: 'sp-3',
          prompt: 'Best time to make checklist?',
          options: <String>[
            'Day before market to prepare',
            'Day of market',
            'Never'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Deep infrequent watering encourages deep roots and saves water versus daily shallow watering.',
    steps: <String>[
      'Test soil 15 cm deep weekly',
      'Water only when soil feels dry',
      'Plan around rain forecasts',
      'Water early morning for efficiency'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'is-1',
          prompt: 'When water deeply?',
          options: <String>[
            'Less often but deeper to encourage roots',
            'Shallow daily',
            'Random scheduling'
          ],
          correctOptionIndex: 0,
          explanation: 'Deep roots access more water.'),
      LessonQuestion(
          id: 'is-2',
          prompt: 'Best test for soil moisture?',
          options: <String>[
            'Squeeze handful - crumbly is dry',
            'Guess',
            'Never test'
          ],
          correctOptionIndex: 0,
          explanation: 'Simple test guides watering decisions.'),
      LessonQuestion(
          id: 'is-3',
          prompt: 'Why check rain forecast?',
          options: <String>[
            'Avoid irrigating before rain',
            'Ignore weather',
            'Water anyway'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Removing excess flowers or fruits concentrates plant resources into larger, higher-quality specimens.',
    steps: <String>[
      'Monitor fruit set carefully',
      'Identify clusters with excess',
      'Remove smaller or damaged fruit',
      'Leave space between remaining fruit'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'tr-1',
          prompt: 'Why trim fruit?',
          options: <String>[
            'Concentrates resources for larger fruit',
            'Waste effort',
            'Never needed'
          ],
          correctOptionIndex: 0,
          explanation: 'Fewer fruits = bigger, better quality.'),
      LessonQuestion(
          id: 'tr-2',
          prompt: 'What to remove?',
          options: <String>[
            'Smaller or damaged fruit, excess blooms',
            'Keep everything',
            'Remove largest'
          ],
          correctOptionIndex: 0,
          explanation: 'Selection favors best specimens.'),
      LessonQuestion(
          id: 'tr-3',
          prompt: 'Result of proper trimming?',
          options: <String>[
            'Better market price, customer satisfaction',
            'No change',
            'Lower quality'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Regular feed inspections prevent illness from moldy or contaminated batches.',
    steps: <String>[
      'Inspect visually for discoloration',
      'Smell for musty or off-odors',
      'Check for pest signs',
      'Discard suspect feed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'fc-1',
          prompt: 'Signs of spoiled feed?',
          options: <String>[
            'Mold, bad odor, discoloration',
            'No signs matter',
            'Always feed'
          ],
          correctOptionIndex: 0,
          explanation: 'These signs indicate spoilage.'),
      LessonQuestion(
          id: 'fc-2',
          prompt: 'Moldy feed consequence?',
          options: <String>[
            'Causes illness and reduced growth',
            'No problem',
            'Improves nutrition'
          ],
          correctOptionIndex: 0,
          explanation: 'Mold toxins harm animal health.'),
      LessonQuestion(
          id: 'fc-3',
          prompt: 'Best feed storage?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Consistent sampling at fixed points provides trends for control decisions versus spotty observations.',
    steps: <String>[
      'Mark fixed sampling points in field',
      'Visit same time weekly',
      'Record pest counts at each',
      'Compare week to week for trends'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ps-1',
          prompt: 'Why fixed points matter?',
          options: <String>[
            'Compare trends week to week',
            'Random sampling is better',
            'No difference'
          ],
          correctOptionIndex: 0,
          explanation: 'Consistency reveals pest population changes.'),
      LessonQuestion(
          id: 'ps-2',
          prompt: 'Best sampling time?',
          options: <String>[
            'Early morning, same time weekly',
            'Random',
            'Afternoon'
          ],
          correctOptionIndex: 0,
          explanation: 'Timing consistency improves accuracy.'),
      LessonQuestion(
          id: 'ps-3',
          prompt: 'Value of scouting?',
          options: <String>[
            'Avoids unnecessary spraying, saves cost',
            'Always spray',
            'No benefit'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Sales records create accountability and build buyer confidence through transparency.',
    steps: <String>[
      'Record buyer name and contact',
      'Note date, quantity, price',
      'Document payment method received',
      'File for reference and disputes'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sd-1',
          prompt: 'Key record items?',
          options: <String>[
            'Buyer, date, quantity, price, payment',
            'Just quantity',
            'No records needed'
          ],
          correctOptionIndex: 0,
          explanation: 'Complete records prevent misunderstandings.'),
      LessonQuestion(
          id: 'sd-2',
          prompt: 'Record benefits?',
          options: <String>[
            'Protects both seller and buyer',
            'Wastes time',
            'No benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Documentation builds trust.'),
      LessonQuestion(
          id: 'sd-3',
          prompt: 'When disputes arise?',
          options: <String>[
            'Records provide proof and resolution',
            'Guess amounts',
            'Accept loss'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Bare soil erodes quickly; cover crops or residue protect and build soil.',
    steps: <String>[
      'Walk field and observe cover',
      'Note bare patches',
      'Add residue mulch if needed',
      'Plan cover crop for next season'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sc-1',
          prompt: 'Bare soil risks?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Unprotected soil degrades quickly.'),
      LessonQuestion(
          id: 'sc-2',
          prompt: 'How to cover bare patches?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Coverage prevents erosion.'),
      LessonQuestion(
          id: 'sc-3',
          prompt: 'Cover benefits?',
          options: <String>[
            'Prevents erosion, improves soil',
            'No benefit',
            'Negative'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Quick repairs prevent water waste and maintain uniform crop watering.',
    steps: <String>[
      'Carry spare emitters and connectors',
      'Identify leaks regularly',
      'Replace or clean blocked parts',
      'Test repaired zones immediately'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ir-1',
          prompt: 'Why keep spares?',
          options: <String>[
            'Fix breaks immediately, avoid water loss',
            'Unnecessary',
            'Fix later'
          ],
          correctOptionIndex: 0,
          explanation: 'Spares enable quick repairs.'),
      LessonQuestion(
          id: 'ir-2',
          prompt: 'Signs of blocked emitter?',
          options: <String>[
            'Uneven water output, dry patches',
            'No signs',
            'Works fine'
          ],
          correctOptionIndex: 0,
          explanation: 'Blockage causes uneven watering.'),
      LessonQuestion(
          id: 'ir-3',
          prompt: 'Leak impacts?',
          options: <String>[
            'Water waste and increased cost',
            'No problem',
            'Beneficial'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Regular feeding times stabilize appetite and digestion and allow animals to anticipate meals.',
    steps: <String>[
      'Establish two feeding times daily',
      'Measure consistent feed amounts',
      'Maintain schedule even on holidays',
      'Adjust for season changes gradually'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'fs-1',
          prompt: 'Feeding schedule benefits?',
          options: <String>[
            'Stabilizes appetite and digestion',
            'No difference',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Consistency improves intake and growth.'),
      LessonQuestion(
          id: 'fs-2',
          prompt: 'How many times daily?',
          options: <String>[
            'Two feeding times for consistency',
            'Once',
            'Random'
          ],
          correctOptionIndex: 0,
          explanation: 'Regular rhythm trains animals.'),
      LessonQuestion(
          id: 'fs-3',
          prompt: 'Schedule importance?',
          options: <String>[
            'Predictability improves animal health',
            'No benefit',
            'Negative'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Quality grading lets farmers access premium buyer segments instead of bulk discount buyers.',
    steps: <String>[
      'Establish quality grades for crop',
      'Sort produce by size/color/blemishes',
      'Pack separately by grade',
      'Sell premium separately for higher price'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ms-1',
          prompt: 'Why sort by quality?',
          options: <String>[
            'Access premium buyers and prices',
            'Wastes effort',
            'No difference'
          ],
          correctOptionIndex: 0,
          explanation: 'Grading increases value.'),
      LessonQuestion(
          id: 'ms-2',
          prompt: 'Grade criteria?',
          options: <String>[
            'Size, color, firmness, blemishes',
            'Random',
            'No criteria'
          ],
          correctOptionIndex: 0,
          explanation: 'Consistent standards guide sorting.'),
      LessonQuestion(
          id: 'ms-3',
          prompt: 'Market advantage?',
          options: <String>[
            'Premium grade fetches higher price',
            'All same price',
            'Lower for sorted'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Secure housing and guard animals significantly reduce predator losses versus open grazing.',
    steps: <String>[
      'Build secure night housing',
      'Use predator-proof fencing',
      'Consider guard animals',
      'Use lights or noise deterrents'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pc-1',
          prompt: 'Best predator prevention?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Confinement prevents access.'),
      LessonQuestion(
          id: 'pc-2',
          prompt: 'Guard animal roles?',
          options: <String>[
            'Warn and defend against predators',
            'Useless',
            'Just feed waste'
          ],
          correctOptionIndex: 0,
          explanation: 'Guard animals protect flocks.'),
      LessonQuestion(
          id: 'pc-3',
          prompt: 'Deterrent types?',
          options: <String>[
            'Lights, noise, movement scare predators',
            'Nothing works',
            'No use'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Elevated racks with mesh allow airflow underneath for faster, more uniform drying.',
    steps: <String>[
      'Build simple wooden frame',
      'Add mesh or shade cloth',
      'Elevate above ground',
      'Turn produce daily for even drying'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'dr-1',
          prompt: 'Why elevate racks?',
          options: <String>[
            'Allows airflow underneath for faster drying',
            'Looks nice',
            'No reason'
          ],
          correctOptionIndex: 0,
          explanation: 'Air circulation speeds drying.'),
      LessonQuestion(
          id: 'dr-2',
          prompt: 'Mesh material purpose?',
          options: <String>[
            'Allows air while supporting produce',
            'Aesthetic',
            'No purpose'
          ],
          correctOptionIndex: 0,
          explanation: 'Mesh prevents damage while drying.'),
      LessonQuestion(
          id: 'dr-3',
          prompt: 'Turning frequency?',
          options: <String>['Daily for even drying', 'Never', 'Once per week'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Alternating green and brown materials accelerates decomposition compared to random mixing.',
    steps: <String>[
      'Collect green (nitrogen) materials',
      'Collect brown (carbon) materials',
      'Layer alternately',
      'Water and turn regularly'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'cl-1',
          prompt: 'Green materials examples?',
          options: <String>[
            'Grass clippings, manure, food scraps',
            'Only brown',
            'No greens'
          ],
          correctOptionIndex: 0,
          explanation: 'Greens provide nitrogen.'),
      LessonQuestion(
          id: 'cl-2',
          prompt: 'Brown materials examples?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Browns provide carbon.'),
      LessonQuestion(
          id: 'cl-3',
          prompt: 'Layering benefit?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Proper spacing allows light penetration and airflow reducing disease and competition.',
    steps: <String>[
      'Know recommended spacing for crop',
      'Mark rows consistently',
      'Space plants or seeds',
      'Thin excess seedlings'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ss-1',
          prompt: 'Spacing importance?',
          options: <String>[
            'Reduces competition and disease',
            'No difference',
            'Too complicated'
          ],
          correctOptionIndex: 0,
          explanation: 'Spacing improves health and yield.'),
      LessonQuestion(
          id: 'ss-2',
          prompt: 'Air circulation benefit?',
          options: <String>[
            'Dries leaves, prevents fungal disease',
            'No benefit',
            'Wastes water'
          ],
          correctOptionIndex: 0,
          explanation: 'Airflow prevents diseases.'),
      LessonQuestion(
          id: 'ss-3',
          prompt: 'Crowded plant result?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Simple water tests identify contamination before use on crops or animals.',
    steps: <String>[
      'Collect sample in clean container',
      'Check for cloudiness or color',
      'Smell for odors',
      'Observe floating matter'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'wt-1',
          prompt: 'Contamination signs?',
          options: <String>[
            'Cloudiness, odor, color, matter',
            'No signs',
            'Always safe'
          ],
          correctOptionIndex: 0,
          explanation: 'Visual signs indicate problems.'),
      LessonQuestion(
          id: 'wt-2',
          prompt: 'Test importance?',
          options: <String>[
            'Prevents crop/animal harm',
            'No benefit',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Testing protects farm.'),
      LessonQuestion(
          id: 'wt-3',
          prompt: 'Contaminated water use?',
          options: <String>['Discard or treat', 'Use anyway', 'No problem'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Careful packing prevents bruising and quality loss that reduce market price.',
    steps: <String>[
      'Layer delicate produce with padding',
      'Avoid mixing hard and soft items',
      'Fill crates firmly to prevent shifting',
      'Label and stack safely'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'mp-1',
          prompt: 'Padding purpose?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Protection maintains quality.'),
      LessonQuestion(
          id: 'mp-2',
          prompt: 'Why separate by type?',
          options: <String>[
            'Prevents bruising of sensitive produce',
            'No reason',
            'Mix everything'
          ],
          correctOptionIndex: 0,
          explanation: 'Mixing causes damage.'),
      LessonQuestion(
          id: 'mp-3',
          prompt: 'Proper crate fill?',
          options: <String>[
            'Firm and full to prevent shifting',
            'Loose',
            'Leave space'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Regular pasture assessment prevents overgrazing damage and maintains feed quality.',
    steps: <String>[
      'Measure forage height regularly',
      'Check for bare spots',
      'Monitor plant health',
      'Rotate grazing areas before overuse'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pc-1',
          prompt: 'Overgrazing impacts?',
          options: <String>[
            'Soil degradation and poor recovery',
            'No problem',
            'Improves pasture'
          ],
          correctOptionIndex: 0,
          explanation: 'Overgrazing damages land.'),
      LessonQuestion(
          id: 'pc-2',
          prompt: 'Rotation benefits?',
          options: <String>[
            'Allows recovery and prevents degradation',
            'No benefit',
            'Wastes pasture'
          ],
          correctOptionIndex: 0,
          explanation: 'Rest allows regrowth.'),
      LessonQuestion(
          id: 'pc-3',
          prompt: 'Ideal forage height?',
          options: <String>[
            '5-10 cm for cattle, varies by species',
            'No height matters',
            'Always short'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Curing heals harvest wounds in root crops improving storage and preventing rot.',
    steps: <String>[
      'Harvest and store in warm shade',
      'Maintain 70-80°F and 85-90% humidity',
      'Wait 2-3 weeks for wound healing',
      'Move to cool storage afterward'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'cur-1',
          prompt: 'Curing purpose?',
          options: <String>[
            'Heals wounds, prevents rot, extends storage',
            'No benefit',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Curing improves shelf life significantly.'),
      LessonQuestion(
          id: 'cur-2',
          prompt: 'Ideal cure temperature?',
          options: <String>[
            '70-80°F in humid conditions',
            'Cold storage',
            'Hot and dry'
          ],
          correctOptionIndex: 0,
          explanation: 'Warm humidity encourages healing.'),
      LessonQuestion(
          id: 'cur-3',
          prompt: 'After curing, do what?',
          options: <String>['Move to cool storage', 'Leave outside', 'Discard'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Simple hand tests accurately indicate soil moisture without meters or equipment.',
    steps: <String>[
      'Take handful of soil from 15cm deep',
      'Feel texture: crumbly means dry',
      'If sticky or forms ball, irrigate soon',
      'Practice to recognize feel'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'smt-1',
          prompt: 'Dry soil feels?',
          options: <String>['Crumbly and falls apart', 'Sticky', 'Soaking wet'],
          correctOptionIndex: 0,
          explanation: 'Crumbling indicates low moisture.'),
      LessonQuestion(
          id: 'smt-2',
          prompt: 'Moist but adequate?',
          options: <String>[
            'Forms ball but falls apart easily',
            'Always wet',
            'Always dry'
          ],
          correctOptionIndex: 0,
          explanation: 'Ball that breaks shows good moisture.'),
      LessonQuestion(
          id: 'smt-3',
          prompt: 'Wet soil action?',
          options: <String>[
            'Do not water, wait for drying',
            'Water more',
            'No difference'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Pre-transport checks prevent breakdowns and accidents during market trips.',
    steps: <String>[
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'vc-1',
          prompt: 'Tire pressure importance?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Proper pressure prevents failures.'),
      LessonQuestion(
          id: 'vc-2',
          prompt: 'Brake check purpose?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Brakes ensure control.'),
      LessonQuestion(
          id: 'vc-3',
          prompt: 'Load securing needed?',
          options: <String>[
            'Prevents spills and shifting',
            'Optional',
            'No benefit'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Proper safety practices prevent poisoning and environmental contamination.',
    steps: <String>[
      'Wear full PPE: gloves, mask, goggles',
      'Check wind direction',
      'Spray early morning when calm',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          ...
          prompt: 'PPE includes?',
          options: <String>[
            'Gloves, mask, goggles, coveralls',
            'No PPE needed',
            'Just shirt'
          ],
          correctOptionIndex: 0,
          explanation: 'Full PPE prevents absorption.'),
      LessonQuestion(
          id: 'ss-2',
          prompt: 'Wind direction importance?',
          options: <String>[
            'Prevents drift to unintended areas',
            'No difference',
            'Ignore wind'
          ],
          correctOptionIndex: 0,
          explanation: 'Wind control prevents drift damage.'),
      LessonQuestion(
          id: 'ss-3',
          prompt: 'Label rate necessity?',
          options: <String>[
            'Prevents overdose and harm',
            'More is better',
            'Ignore labels'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Pre-planting germination tests prevent wasting seed and land on poor lots.',
    steps: <String>[
      'Take 100 seeds from lot',
      'Place on moist paper towel',
      'Keep moist and warm for 7-10 days',
      'Count germinated seeds for viability %'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'st-1',
          prompt: 'Why test seeds?',
          options: <String>[
            'Know viability before planting',
            'Waste time',
            'Always plant'
          ],
          correctOptionIndex: 0,
          explanation: 'Testing prevents crop failure.'),
      LessonQuestion(
          id: 'st-2',
          prompt: 'Good germination rate?',
          options: <String>[
            '85-90%+ for most crops',
            'Any rate works',
            'Only 50%'
          ],
          correctOptionIndex: 0,
          explanation: 'High viability ensures stands.'),
      LessonQuestion(
          id: 'st-3',
          prompt: 'Low viability result?',
          options: <String>[
            'Get better seed or increase rate',
            'Plant anyway',
            'No change needed'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Reliable buyer network reduces marketing effort and uncertainty.',
    steps: <String>[
      'Identify 3-5 reliable buyers',
      'Record phone, location, preferences',
      'Note typical price for crops',
      'Update after each transaction'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'mc-1',
          prompt: 'Buyer list benefit?',
          options: <String>[
            'Quick, reliable market access',
            'Wastes time',
            'No benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Networks reduce marketing cost.'),
      LessonQuestion(
          id: 'mc-2',
          prompt: 'Info to record?',
          options: <String>[
            'Phone, location, crop preferences',
            'Nothing',
            'Just name'
          ],
          correctOptionIndex: 0,
          explanation: 'Details enable quick sales.'),
      LessonQuestion(
          id: 'mc-3',
          prompt: 'Update frequency?',
          options: <String>[
            'After each sale to stay current',
            'Never',
            'Once yearly'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery06,
    overview:
        'Proper storage protects feed quality and reduces losses to pests.',
    steps: <String>[
      'Use sealed, airtight containers',
      'Elevate off floor on pallets',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'fs-1',
          prompt: 'Sealed containers prevent?',
          options: <String>[
            'Moisture and pest entry',
            'No benefit',
            'Unnecessary'
          ],
          correctOptionIndex: 0,
          explanation: 'Sealing preserves quality.'),
      LessonQuestion(
          id: 'fs-2',
          prompt: 'Why elevate storage?',
          options: <String>[
            'Prevents floor moisture and rodents',
            'No reason',
            'Wastes space'
          ],
          correctOptionIndex: 0,
          explanation: 'Height prevents ground contact.'),
      LessonQuestion(
          id: 'fs-3',
          prompt: 'Storage location?',
          options: <String>[
            'Cool and dry to prevent spoilage',
            'Hot and wet',
            'No difference'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Pest thresholds guide control decisions avoiding spraying costs when monitoring suffices.',
    steps: <String>[
      'Know economic threshold for crop',
      'Scout regularly',
      'Count affected plants',
      'Spray only when threshold exceeded'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pt-1',
          prompt: 'Threshold purpose?',
          options: <String>[
            'Guides when to spray for profit',
            'Always spray',
            'Never spray'
          ],
          correctOptionIndex: 0,
          explanation: 'Thresholds improve economics.'),
      LessonQuestion(
          id: 'pt-2',
          prompt: 'Below-threshold pests?',
          options: <String>[
            'Monitor, do not spray yet',
            'Spray anyway',
            'Ignore'
          ],
          correctOptionIndex: 0,
          explanation: 'Monitoring saves costs.'),
      LessonQuestion(
          id: 'pt-3',
          prompt: 'Above-threshold action?',
          options: <String>[
            'Spray or use other control',
            'Ignore',
            'Hope it stops'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Scheduled pruning at growth stages optimizes plant form and productivity.',
    steps: <String>[
      'Identify key growth stages',
      'Plan pruning dates',
      'Record schedule yearly',
      'Adjust based on weather'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'prs-1',
          prompt: 'Pruning timing importance?',
          options: <String>[
            'Growth stage matters for response',
            'Anytime is fine',
            'Never prune'
          ],
          correctOptionIndex: 0,
          explanation: 'Timing affects recovery.'),
      LessonQuestion(
          id: 'prs-2',
          prompt: 'Overgrown result?',
          options: <String>[
            'Reduced fruiting and disease',
            'Better production',
            'No change'
          ],
          correctOptionIndex: 0,
          explanation: 'Dense canopy traps moisture.'),
      LessonQuestion(
          id: 'prs-3',
          prompt: 'Schedule benefit?',
          options: <String>[
            'Ensures consistent maintenance',
            'Wastes time',
            'Unnecessary'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Professional invoices build buyer confidence and reduce payment disputes.',
    steps: <String>[
      'Create simple invoice template',
      'List goods, quantities, unit prices',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'inv-1',
          prompt: 'Invoice purpose?',
          options: <String>[
            'Documents sale and payment terms',
            'Wastes paper',
            'No use'
          ],
          correctOptionIndex: 0,
          explanation: 'Invoices prevent disputes.'),
      LessonQuestion(
          id: 'inv-2',
          prompt: 'Invoice contents?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Details ensure clarity.'),
      LessonQuestion(
          id: 'inv-3',
          prompt: 'Records kept?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    steps: <String>[
      'Select barrier material',
      'Lay over area',
      'Secure edges with mulch or pegs',
      'Cut holes for planting'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'wb-1',
          prompt: 'Barrier types?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Physical barriers block light.'),
      LessonQuestion(
          id: 'wb-2',
          prompt: 'Barrier benefit?',
          options: <String>[
            'Eliminates persistent weeds',
            'Wasted effort',
            'No benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Barriers are effective.'),
      LessonQuestion(
          id: 'wb-3',
          prompt: 'Long-term solution?',
          options: <String>['Yes, can last 5+ years', 'Temporary', 'Worthless'],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery01,
    overview:
        'Disease records reveal patterns enabling early prevention of future outbreaks.',
    steps: <String>[
      'Record date disease appears',
      'Note affected field section',
      'Describe symptoms and extent',
      'Take photo for reference'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'dr-1',
          prompt: 'Record importance?',
          options: <String>[
          ...
          correctOptionIndex: 0,
          explanation: 'Patterns guide prevention.'),
      LessonQuestion(
          id: 'dr-2',
          prompt: 'Include in records?',
          options: <String>[
            'Date, location, symptoms, photos',
            'Just name',
            'Nothing matters'
          ],
          correctOptionIndex: 0,
          explanation: 'Details enable analysis.'),
      LessonQuestion(
          id: 'dr-3',
          prompt: 'Use of records?',
          options: <String>[
            'Plan prevention for next year',
            'Ignore',
            'Forget it'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery02,
    overview:
        'Quality samples give buyers confidence and lead to better offers.',
    steps: <String>[
      'Select average-quality produce',
      'Include variety of sizes',
      'Package neatly in display',
      'Present cleanly at market'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'msp-1',
          prompt: 'Sample represents?',
          options: <String>[
            'True crop quality, not cherry-picked',
            'Only best fruit',
            'Worst examples'
          ],
          correctOptionIndex: 0,
          explanation: 'Honest sampling builds trust.'),
      LessonQuestion(
          id: 'msp-2',
          prompt: 'Presentation matters?',
          options: <String>[
            'Yes, affects buyer perception',
            'No difference',
            'Ignore'
          ],
          correctOptionIndex: 0,
          explanation: 'Appearance influences offers.'),
      LessonQuestion(
          id: 'msp-3',
          prompt: 'Sample benefit?',
          options: <String>[
            'Buyers see quality, offer better price',
            'No impact',
            'Lowers price'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery03,
    overview:
        'Regular parasite checks prevent blood loss and disease from ticks and lice.',
    steps: <String>[
      'Inspect skin and coat',
      'Check ears and around eyes',
      'Look for movement or bites',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          ...
          prompt: 'Tick signs?',
          options: <String>[
            'Embedded in skin, blood loss',
            'Just cosmetic',
            'No problem'
          ],
          correctOptionIndex: 0,
          explanation: 'Ticks feed on blood.'),
      LessonQuestion(
          id: 'pc-2',
          prompt: 'Lice impacts?',
          options: <String>[
            'Itching, hair loss, stress',
            'No effect',
            'Helps animal'
          ],
          correctOptionIndex: 0,
          explanation: 'Lice cause discomfort.'),
      LessonQuestion(
          id: 'pc-3',
          prompt: 'Check frequency?',
          options: <String>[
            'Monthly or after grazing in brush',
            'Never',
            'Once yearly'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery04,
    overview:
        'Mapped zones enable consistent water management and problem identification.',
    steps: <String>[
      'Observe field for variations',
      'Mark high, medium, low areas',
      'Note slope and soil differences',
      'Create simple map reference'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'im-1',
          prompt: 'Zone benefits?',
          options: <String>[
            'Different areas get appropriate water',
            'No difference',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Zones enable tuning.'),
      LessonQuestion(
          id: 'im-2',
          prompt: 'Variation sources?',
          options: <String>[
            'Slope, soil type, drainage',
            'No variations',
            'Ignore'
          ],
          correctOptionIndex: 0,
          explanation: 'Topology affects water.'),
      LessonQuestion(
          id: 'im-3',
          prompt: 'Low zone issue?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery05,
    overview:
        'Calibration ensures target plant population improving uniformity and yield.',
    steps: <String>[
      'Read seeder manual for target rate',
      'Test output over known distance',
      'Count seeds collected',
      'Adjust opening or speed'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'sc-1',
          prompt: 'Calibration importance?',
          options: <String>[
            'Achieves target stand density',
            'No difference',
            'Wastes time'
          ],
          correctOptionIndex: 0,
          explanation: 'Correct population matters.'),
      LessonQuestion(
          id: 'sc-2',
          prompt: 'Test method?',
          options: <String>[
            'Collect seeds over measured run',
            'Guess',
            'No testing'
          ],
          correctOptionIndex: 0,
          explanation: 'Testing provides data.'),
      LessonQuestion(
          id: 'sc-3',
          prompt: 'Adjustment options?',
          options: <String>[
            'Change opening size or speed',
            'Cannot adjust',
            'Try luck'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    steps: <String>[
      'Use straps across crates',
      'Cover loads if needed',
      'Check tie-downs secure',
      'Test before departing'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'tc-1',
          prompt: 'Strap necessity?',
          options: <String>[
            'Prevents shifting and spills',
            'Optional',
            'No need'
          ],
          correctOptionIndex: 0,
          explanation: 'Straps secure cargo.'),
      LessonQuestion(
          id: 'tc-2',
          prompt: 'Cover protects?',
          options: <String>['Rain, wind, UV damage', 'No benefit', 'Trap heat'],
          correctOptionIndex: 0,
          explanation: 'Covers protect produce.'),
      LessonQuestion(
          id: 'tc-3',
          prompt: 'Tie-down check?',
          options: <String>[
            'Ensure tight before trip',
            'Do not check',
            'Loose is OK'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery07,
    overview:
        'Proper safety technique prevents cuts and eye injuries during pruning.',
    steps: <String>[
      'Wear gloves and safety goggles',
      'Cut away from body',
      'Secure tools when not in use',
      'Seek first aid for injuries'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'ps-1',
          prompt: 'Required PPE?',
          options: <String>['Gloves and goggles always', 'No PPE', 'Optional'],
          correctOptionIndex: 0,
          explanation: 'PPE prevents injuries.'),
      LessonQuestion(
          id: 'ps-2',
          prompt: 'Cut direction?',
          options: <String>[
            'Away from body and others',
            'Toward self',
            'Any direction'
          ],
          correctOptionIndex: 0,
          explanation: 'Direction prevents cuts.'),
      LessonQuestion(
          id: 'ps-3',
          prompt: 'Eye protection purpose?',
          options: <String>[
            'Prevents branch and debris injury',
            'Not needed',
            'Unnecessary'
          ],
          correctOptionIndex: 0,
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery08,
    overview:
        'Pre-harvest checks prevent problems and ensure timely, quality harvesting.',
    steps: <String>[
      'Check soil moisture for firmness',
      'Inspect for pests or disease',
      'Ensure workers and tools ready',
      'Start harvest when conditions right'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'hc-1',
          prompt: 'Moisture importance?',
          options: <String>[
            'Affects handling and post-harvest quality',
            'No difference',
            'Irrelevant'
          ],
          correctOptionIndex: 0,
          explanation: 'Moisture influences spoilage.'),
      LessonQuestion(
          id: 'hc-2',
          prompt: 'Pest inspection goal?',
          options: <String>[
            'Avoid packaging infested crop',
            'No inspection',
            'Ignore pests'
          ],
          correctOptionIndex: 0,
          explanation: 'Detection prevents loss.'),
      LessonQuestion(
          id: 'hc-3',
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery09,
    overview:
        'Consistent safety practices significantly reduce farm injuries and illness.',
    steps: <String>[
      'Wear PPE for all fieldwork',
      'Keep tools tidy and secure',
    ...
    questions: <LessonQuestion>[
      LessonQuestion(
          ...
          prompt: 'PPE always?',
          options: <String>[
            'Yes, reduces injury risk',
            'Optional',
            'No PPE needed'
          ],
          correctOptionIndex: 0,
          explanation: 'PPE is protection.'),
      LessonQuestion(
          id: 'fs-2',
          prompt: 'Chemical storage?',
          options: <String>[
            'Locked cabinet away from animals',
            'Open storage',
            'Any location'
          ],
          correctOptionIndex: 0,
          explanation: 'Proper storage prevents poisoning.'),
      LessonQuestion(
          id: 'fs-3',
          prompt: 'Injury response?',
          options: <String>[
  ...
  const LearningLesson(
    ...
    imageAsset: AppAssets.uiGallery10,
    overview:
        'Weekly reflection improves decision-making and ensures continuous farm improvement.',
    steps: <String>[
      'List three wins from week',
      'Note three challenges or losses',
      'Set three specific actions for next week',
      'Record in farm journal'
    ],
    questions: <LessonQuestion>[
      LessonQuestion(
          id: 'pr-1',
          prompt: 'Planning review benefit?',
          options: <String>[
            'Improves decisions, tracks progress',
            'Wastes time',
            'No benefit'
          ],
          correctOptionIndex: 0,
          explanation: 'Reflection drives improvement.'),
      LessonQuestion(
          id: 'pr-2',
          prompt: 'Include in review?',
          options: <String>[
            'Wins, challenges, next actions',
            'Just wins',
            'Random notes'
          ],
          correctOptionIndex: 0,
          explanation: 'Balanced review is effective.'),
      LessonQuestion(
          id: 'pr-3',
          prompt: 'Action setting importance?',
          options: <String>[
            'Moves farm forward progressively',
            'No action needed',
            'Just review'
          ],
          correctOptionIndex: 0,
...
const List<LearningTrack> _learningTracks = <LearningTrack>[
  LearningTrack(
    ...
    summary:
        'Crop planning, field care, growth stages, pest scouting, and harvest-readiness lessons.',
  ),
  LearningTrack(
    ...
    summary:
        'Vaccination routines, hygiene, feeding, breeding, and disease response basics.',
  ),
  LearningTrack(
    ...
    summary:
        'Moisture, fertility, structure, erosion prevention, and practical soil observations.',
  ),
  LearningTrack(
    ...
    summary:
        'Weather interpretation, timing decisions, and work planning around field conditions.',
  ),
  LearningTrack(
    title: 'Market access',
    icon: Icons.storefront_rounded,
    ...
    summary:
        'Market timing, buyer comparison, receipt records, procurement, and stock decisions.',
  ),
];
...
const List<PracticeActivity> _practiceActivities = <PracticeActivity>[
  PracticeActivity(
    ...
    subtitle:
        'Walk crop rows and mark irrigation stress, weed pressure, and flowering changes.',
    icon: Icons.fact_check_rounded,
  ...
  PracticeActivity(
    ...
    title: 'Weekly quiz',
    subtitle:
        'A quick five-question check on disease prevention and livestock hygiene.',
  ...
  PracticeActivity(
    ...
    subtitle:
        'Save what worked on your farm so the next lesson stays practical.',
  ...
  PracticeActivity(
    ...
    title: 'Market review',
    subtitle:
        'Compare last sale price, buyer reliability, transport cost, and remaining inventory.',
    icon: Icons.receipt_long_rounded,
    color: Color(0xFFEDE8FF),
    whyItMatters:
